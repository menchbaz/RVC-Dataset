...
if not input_files:
        return "خطا: هیچ فایل صوتی‌ای پیدا نشد!"
...
def process_audio(echo_reduction=0.9, presence=0.1): try: input_path = "output/combined_vocals.wav" if not os.path.exists(input_path): return "لطفا ابتدا از بخش ترکیب صداها استفاده کنید"
...
with gr.Blocks(title="پردازشگر حرفه‌ای صدا") as app: gr.Markdown("# 🎵 پردازشگر حرفه‌ای صدا") with gr.Tab("جداسازی صدا"): url_input = gr.Textbox(label="لینک ویدیو (اختیاری)") file_input = gr.File(file_count="multiple", label="آپلود فایل‌ها (اختیاری)") model_choice = gr.Dropdown( choices=["BS-Roformer-1297", "BS-Roformer-1296", "Mel-Roformer-1143"], label="انتخاب مدل", value="BS-Roformer-1297" ) separate_button = gr.Button("شروع جداسازی") separate_output = gr.Textbox(label="نتیجه") separate_button.click(separate_audio, [url_input, file_input, model_choice], separate_output)
...
with gr.Tab("پردازش نهایی"):
    echo_slider = gr.Slider(0.7, 0.95, 0.9, label="میزان حذف اکو")
    presence_slider = gr.Slider(0.1, 0.3, 0.1, label="میزان حضور صدا")
